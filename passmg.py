...
def Main():
	...
	if len(sys.argv)  != 2:
		...
		try:
			...
			pws = decrypt(k)#the password is decripted incase the database does not have the password
			pws = bytesToDict(pws)
			...
			global x
			x = x-1
			print("The password is incorrect. You have "+ str(x) +" attempts remaining.")
